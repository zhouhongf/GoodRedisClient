...
class NewKeyRequest {
 ...
 public:
  NewKeyRequest(QSharedPointer<RedisClient::Connection> connection, int dbIndex, std::function<void()> callback, QString keyPrefix = QString())
  ...
  QString dbIdString() {
    return QString("%1:db%2").arg(m_connection->getConfig().name()).arg(m_dbIndex);
  }
