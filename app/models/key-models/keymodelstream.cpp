...
StreamKeyModel::StreamKeyModel(QSharedPointer<RedisClient::Connection> connection, QByteArray fullPath, int dbIndex, long long ttl): KeyModel(connection, fullPath, dbIndex, ttl, "XLEN", QByteArray()) {}

QString StreamKeyModel::type() { return "stream"; }

QStringList StreamKeyModel::getColumnNames() {
  return QStringList() << "rowNumber" << "id" << "value";
}
...
QVariant StreamKeyModel::getData(int rowIndex, int dataRole) {
  ...
  switch (dataRole) {
    case Value:
      return QJsonDocument::fromVariant(m_rowsCache[rowIndex].second).toJson(QJsonDocument::Compact);
    case ID:
...
void StreamKeyModel::updateRow(int, const QVariantMap &, ValueEditor::Model::Callback) {
    //NOTE(u_glide): Redis Streams doesn't support editing (yet?)
}


void StreamKeyModel::addRow(const QVariantMap &row, ValueEditor::Model::Callback c) {
  if (!isRowValid(row)) {
  ...
  QJsonParseError err;
  QJsonDocument jsonValues = QJsonDocument::fromJson(row["value"].toByteArray(), &err);
...
void StreamKeyModel::loadRowsCount(ValueEditor::Model::Callback c) {
  executeCmd({"XINFO", "STREAM", m_keyFullPath},
             c,
             [this](RedisClient::Response r, Callback c) {
                ...
                while (it != info.end()) {
                  ...
                  it++;
                  if (it == info.end()){ break; }

                  if (propertyName == QByteArray("length")) {
                    m_rowCount = it->toLongLong();
                  } else if (propertyName == QByteArray("first-entry") || propertyName == QByteArray("last-entry")) {
                    auto list = it->toList();
                    if (list.size() > 0) {
...
int StreamKeyModel::addLoadedRowsToCache(const QVariantList &rows, QVariant rowStartId) {
  ...
  for (QVariantList::const_iterator item = rows.begin(); item != rows.end(); ++item) {
    ...
    for (QVariantList::const_iterator valItem = valuesList.begin(); valItem != valuesList.end(); ++valItem) {
...
QList<QByteArray> StreamKeyModel::getRangeCmd(QVariant rowStartId, unsigned long count) {
