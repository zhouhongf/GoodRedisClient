...
SortedSetKeyModel::SortedSetKeyModel(QSharedPointer<RedisClient::Connection> connection, QByteArray fullPath, int dbIndex, long long ttl) : KeyModel(connection, fullPath, dbIndex, ttl, "ZCARD", "ZRANGE WITHSCORES") {}

QString SortedSetKeyModel::type() { return "zset"; }

QHash<int, QByteArray> SortedSetKeyModel::getRoles() {
  QHash<int, QByteArray> roles;
  roles[Roles::RowNumber] = "rowNumber";    // index
  roles[Roles::Value] = "value";            // first
  roles[Roles::Score] = "score";            // second
  return roles;
}

QStringList SortedSetKeyModel::getColumnNames() {
  return QStringList() << "rowNumber" << "value" << "score";
}
...
void SortedSetKeyModel::updateRow(int rowIndex, const QVariantMap &row, Callback c) {
  ...
  QByteArray rowvalue = (valueChanged) ? row["value"].toByteArray() : cachedRow.first;
  QByteArray rowscore = (scoreChanged) ? row["score"].toByteArray() : cachedRow.second;
  QPair<QByteArray, QByteArray> newRow(rowvalue, rowscore);
  ...
  if (valueChanged) {
    deleteSortedSetRow(cachedRow.first,
                       [this, c, onRowAdded, newRow](const QString &err) {
...
void SortedSetKeyModel::addRow(const QVariantMap &row, Callback c) {
  ...
  addSortedSetRow(row["value"].toByteArray(), row["score"].toByteArray(), onAdded);
}
...
void SortedSetKeyModel::removeRow(int i, Callback c) {
  ...
  executeCmd({"ZREM", m_keyFullPath, value},
             [this, c, i](const QString &err) {
                if (err.isEmpty()) {
                  m_rowCount--;
...
void SortedSetKeyModel::addSortedSetRow(const QByteArray &value, QByteArray score, Callback c, bool updateExisting) {
...
void SortedSetKeyModel::deleteSortedSetRow(const QByteArray &value, Callback c) {
...
int SortedSetKeyModel::addLoadedRowsToCache(const QVariantList &rows, QVariant rowStartId) {
